...
uv_counts_perm <- adonis(uv_counts_dist ~ time*treatment, data=uv_counts, strata = uv_counts$time, permutations = 9999, parallel = getOption("mc.cores"))
uv_counts_perm
...
uv_counts_perm_out
write.csv(uv_counts_perm_out, file = "sctld uv counts PERMANOVA pair.csv")
...
# creates a dissimilarity matrix of square-root transformed variables, based on Euclidean distance
uv_pam_dist <- vegdist(uv_pam[c(7:8)], method="euclidian", na.rm= TRUE)

# testing for homogeneity of variance among locations
uv_pam_disp <- betadisper(uv_pam_dist, group=uv_pam$treatment)
permutest(uv_pam_disp, bias.adjust = TRUE, perm = 9999)
# insignificant test value indicates homogeneous variance

# running the PERMANOVA
# time is accounted as a random factor using 'strata'
uv_pam_perm <- adonis(uv_pam_dist ~ time*treatment, data=uv_pam, strata = uv_pam$time, permutations = 9999, parallel = getOption("mc.cores"))
uv_pam_perm

write.csv(uv_pam_perm$aov.tab, file = "sctld uv PAM PERMANOVA.csv")

# pairwise PERMANOVAs
uv_pam_perm_time <- pairwise.adonis2(uv_pam_dist ~ time, data = uv_pam)
uv_pam_perm_time

uv_pam_perm_treat <- pairwise.adonis2(uv_pam_dist ~ treatment, data = uv_pam)
uv_pam_perm_treat

uv_pam_perm_int <- pairwise.adonis2(uv_pam_dist ~ time_treatment, data = uv_pam)
uv_pam_perm_int

# exporting pairwise results
uv_pam_perm_out <- bind_rows(uv_pam_perm_time$t1_vs_t2, uv_pam_perm_time$t1_vs_t3, uv_pam_perm_time$t1_vs_t4, uv_pam_perm_time$t2_vs_t3, uv_pam_perm_time$t2_vs_t4, uv_pam_perm_time$t3_vs_t4, uv_pam_perm_treat$HW_vs_DW, uv_pam_perm_treat$HW_vs_UV, uv_pam_perm_treat$DW_vs_UV,  .id = "Comparison")
uv_pam_perm_out
write.csv(uv_pam_perm_out, file = "sctld uv PAM PERMANOVA pair.csv")
...
par(mfrow=c(1,2))
plot(vectors[,1], vectors[,2],col=c("#80cdc1","#a6611a","#dfc27d")[as.numeric(as.factor(uv_counts$treatment))],pch=c(15,1,19,17)[as.numeric((as.factor(uv_counts$time)))], xlab="Coordinate 1", ylab="Coordinate 2", main="Treatment")
# ordispider(vectors, uv_counts$treatment, label=F, col=c("#80cdc1","#a6611a","#dfc27d"))
legend("topright", legend=c("HW", "DW","UV"), fill = c("#80cdc1","#a6611a","#dfc27d"), bty="n")
...
legend("bottomright", legend=c("HW", "DW","UV"), pch=c(15,18,25,3), bty="n")
dev.off()
...
uv_counts_perm_stats <- data.frame(time_treatment = "t4_DW",abundance = 0,lab = "time: F3,35=28.0914, p=0.002",time = "t4",taxa="other")
uv_counts_perm_stats2 <- data.frame(time_treatment = "t4_DW",abundance = 0,lab = "treat: F2,35=0.5732, p=0.75",time = "t4",taxa="other")
...
ggsave("sctld uv counts abundance plot.pdf", plot= uv_count_abundance, width=10, height=4, units="in", dpi=300)
...
# creates dummy dataframes for custom text labels of stats outputs
uv_counts_stats <- data.frame(time_treatment = "t4_DW",lab = "time: F3,35=69.614, p<0.001",time = "t4")
uv_counts_stats2 <- data.frame(time_treatment = "t4_DW",lab = "treat: F2,35=0.164, p=0.85",time = "t4")
...
uv_counts_plot <-
  ggboxplot(
    ...
    y = "live",
    color = "grey30",
    palette = c("#80cdc1","#a6611a","#dfc27d","#80cdc1","#a6611a","#dfc27d","#80cdc1","#a6611a","#dfc27d","#80cdc1","#a6611a","#dfc27d","#80cdc1","#a6611a","#dfc27d"),
    fill = "time_treatment",
    add = "jitter",
...
ggsave("sctld uv counts plot.pdf", plot= uv_counts_plot, width=10, height=4, units="in", dpi=300)
...
uv_pam_cld_Fo$Group  <- factor(uv_pam_cld_Fo$Group , levels = c("t1_HW","t1_DW", "t1_UV","t2_HW","t2_DW", "t2_UV","t3_HW","t3_DW", "t3_UV","t4_HW","t4_DW", "t4_UV"))
...
uv_pam_cld_Fo <- uv_pam_cld_Fo %>% add_row(Group = "t0_UV", time_treatment = "t0_UV", time = "t0", .before = T)
uv_pam_cld_Fo <- uv_pam_cld_Fo %>% add_row(Group = "t0_DW", time_treatment = "t0_DW", time = "t0", Letter = "no data", .before = T)
uv_pam_cld_Fo <- uv_pam_cld_Fo %>% add_row(Group = "t0_HW", time_treatment = "t0_HW", time = "t0", .before = T)
uv_pam_cld_Fo

# creates dummy dataframes for custom text labels of stats outputs
uv_pam_Fo_stats <- data.frame(time_treatment = "t0_DW",lab = "time: F3,179=207.5, p<0.001",time = "t0")
uv_pam_Fo_stats2 <- data.frame(time_treatment = "t0_DW",lab = "treat: F2,179=200.9, p<0.001",time = "t0")
uv_pam_Fo_stats3 <- data.frame(time_treatment = "t0_DW",lab = "int: F6,179=261.4, p<0.001",time = "t0")
...
ggsave("sctld uv PAM Fo plot.pdf", plot= uv_pam_Fo_plot, width=10, height=4, units="in", dpi=300)
...
uv_pam_cld_Fv$Group  <- factor(uv_pam_cld_Fv$Group , levels = c("t1_HW","t1_DW", "t1_UV","t2_HW","t2_DW", "t2_UV","t3_HW","t3_DW", "t3_UV","t4_HW","t4_DW", "t4_UV"))
...
uv_pam_cld_Fv <- uv_pam_cld_Fv %>% add_row(Group = "t0_UV", time_treatment = "t0_UV", time = "t0", .before = T)
uv_pam_cld_Fv <- uv_pam_cld_Fv %>% add_row(Group = "t0_DW", time_treatment = "t0_DW", time = "t0", Letter = "no data", .before = T)
uv_pam_cld_Fv <- uv_pam_cld_Fv %>% add_row(Group = "t0_HW", time_treatment = "t0_HW", time = "t0", .before = T)
uv_pam_cld_Fv

# creates dummy dataframes for custom text labels of stats outputs
uv_pam_Fv_stats <- data.frame(time_treatment = "t0_DW",lab = "time: F3,179=618.17, p<0.001",time = "t0")
uv_pam_Fv_stats2 <- data.frame(time_treatment = "t0_DW",lab = "treat: F2,179=298.2, p<0.001",time = "t0")
uv_pam_Fv_stats3 <- data.frame(time_treatment = "t0_DW",lab = "int: F6,179=40.79, p<0.001",time = "t0")
...
ggsave("sctld uv PAM Fv plot.pdf", plot= uv_pam_Fv_plot, width=10, height=4, units="in", dpi=300)

# cell counts/PAM multiplot
multiplot=grid.arrange(uv_count_abundance, uv_counts_plot, uv_pam_Fo_plot, uv_pam_Fv_plot, ncol=1, nrow=4, widths=c(10), heights=c(4,4,4,4))
...
# running the PERMANOVA
# time is accounted as a random factor using 'strata'
ballast_counts_perm <- adonis(ballast_counts_dist ~ time*treatment, data=ballast_counts, strata = ballast_counts$time, permutations = 9999, parallel = getOption("mc.cores"))
ballast_counts_perm
...
# pairwise PERMANOVA
ballast_counts_perm_treat <- pairwise.adonis2(ballast_counts_dist ~ treatment, data = ballast_counts)
ballast_counts_perm_treat
...
# exporting pairwise results
ballast_counts_perm_out <- bind_rows(ballast_counts_perm_treat$HW120_vs_DW24, ballast_counts_perm_treat$HW120_vs_DW120, ballast_counts_perm_treat$DW24_vs_DW120,  .id = "Comparison")
ballast_counts_perm_out
write.csv(ballast_counts_perm_out, file = "sctld ballast counts PERMANOVA pair.csv")


pairwise.adonis(ballast_counts_dist,ballast_counts$time_treatment)
...
write.csv(ballast_counts_tukey$`time:treatment`, file = "sctld ballast counts Tukey.csv")
...
#### ballast PAM PERMANOVA ####

# creates a dissimilarity matrix of square-root transformed variables, based on Euclidean distance
ballast_pam_dist <- vegdist(ballast_pam[c(7:8)], method="euclidian", na.rm= TRUE)

# testing for homogeneity of variance among locations
ballast_pam_disp <- betadisper(ballast_pam_dist, group=ballast_pam$treatment)
permutest(ballast_pam_disp, bias.adjust = TRUE, perm = 9999)
# insignificant test value indicates homogeneous variance

# running the PERMANOVA
# time is accounted as a random factor using 'strata'
ballast_pam_perm <- adonis(ballast_pam_dist ~ time*treatment, data=ballast_pam, strata = ballast_pam$time, permutations = 9999, parallel = getOption("mc.cores"))
ballast_pam_perm

write.csv(ballast_pam_perm$aov.tab, file = "sctld ballast PAM PERMANOVA.csv")

# pairwise PERMANOVAs
ballast_pam_perm_treat <- pairwise.adonis2(ballast_pam_dist ~ treatment, data = ballast_pam)
ballast_pam_perm_treat

ballast_pam$time_treatment <- paste(ballast_pam$time,ballast_pam$treatment, sep = "_")

ballast_pam_perm_int <- pairwise.adonis2(ballast_pam_dist ~ time_treatment, data = ballast_pam)
ballast_pam_perm_int

# exporting pairwise results
ballast_pam_perm_out <- bind_rows(ballast_pam_perm_treat$HW120_vs_DW24, ballast_pam_perm_treat$HW120_vs_DW120, ballast_pam_perm_treat$DW24_vs_DW120, ballast_pam_perm_int$t0_HW120_vs_t0_DW24, ballast_pam_perm_int$t0_HW120_vs_t0_DW120, ballast_pam_perm_int$t0_HW120_vs_t1_HW120, ballast_pam_perm_int$t0_HW120_vs_t1_DW24, ballast_pam_perm_int$t0_HW120_vs_t1_DW120, ballast_pam_perm_int$t0_DW24_vs_t0_DW120, ballast_pam_perm_int$t0_DW24_vs_t1_HW120, ballast_pam_perm_int$t0_DW24_vs_t1_DW24, ballast_pam_perm_int$t0_DW24_vs_t1_DW120, ballast_pam_perm_int$t0_DW120_vs_t1_HW120, ballast_pam_perm_int$t0_DW120_vs_t1_DW24, ballast_pam_perm_int$t0_DW120_vs_t1_DW120, ballast_pam_perm_int$t1_HW120_vs_t1_DW24, ballast_pam_perm_int$t1_HW120_vs_t1_DW120, ballast_pam_perm_int$t1_DW24_vs_t1_DW120, .id = "Comparison")
ballast_pam_perm_out
write.csv(ballast_pam_perm_out, file = "sctld ballast PAM PERMANOVA pair.csv")
...
par(mfrow=c(1,2))
plot(vectors[,1], vectors[,2],col=c("#80cdc1","#dfc27d","#a6611a")[as.numeric(as.factor(ballast_counts$treatment))],pch=c(15,1)[as.numeric((as.factor(ballast_counts$time)))], xlab="Coordinate 1", ylab="Coordinate 2", main="Treatment")
# ordispider(vectors, ballast_counts$treatment, label=F, col=c("#80cdc1","#dfc27d","#a6611a"))
legend("topright", legend=c("HW120", "DW24","DW120"), fill = c("#80cdc1","#dfc27d","#a6611a"), bty="n")
...
legend("bottomright", legend=c("HW120", "DW24","DW120"), pch=c(15,18,25), bty="n")
dev.off()
...
ballast_counts_perm_letters$comparison = c("DW24-HW120","DW120-HW120","DW120-DW24")
...
# creates dummy dataframes for custom text labels of stats outputs
ballast_counts_perm_stats <- data.frame(time_treatment = "t1_HW120",abundance = 0,lab = "time: F1,17=2.976, p<0.001",time = "t1",taxa="other")
ballast_counts_perm_stats2 <- data.frame(time_treatment = "t1_HW120",abundance = 0,lab = "treat: F2,17=4.071, p=0.001",time = "t1",taxa="other")
ballast_counts_perm_stats3 <- data.frame(time_treatment = "t1_HW120",abundance = 0,lab = "int: F2,17=2.729, p=0.018",time = "t1",taxa="other")
...
ggsave("sctld ballast counts abundance plot.pdf", plot= ballast_count_abundance, width=6, height=4, units="in", dpi=300)
...
ballast_counts_cld$Group  <- factor(ballast_counts_cld$Group , levels = c("t0_HW120","t0_DW24", "t0_DW120","t1_HW120","t1_DW24", "t1_DW120"))
...
# creates dummy dataframes for custom text labels of stats outputs
ballast_counts_stats <- data.frame(time_treatment = "t1_HW120",lab = "time: F1,17=4.895, p=0.047",time = "t1")
ballast_counts_stats2 <- data.frame(time_treatment = "t1_HW120",lab = "treat: F2,17=12.54, p=0.001",time = "t1")
ballast_counts_stats3 <- data.frame(time_treatment = "t1_HW120",lab = "int: F2,17=6.433, p=0.013",time = "t1")
...
ggsave("sctld ballast counts plot.pdf", plot= ballast_counts_plot, width=6, height=4, units="in", dpi=300)
...
ballast_pam_cld_Fo$Group  <- factor(ballast_pam_cld_Fo$Group , levels = c("t0_HW120","t0_DW24", "t0_DW120","t1_HW120","t1_DW24", "t1_DW120"))
...
# creates dummy dataframes for custom text labels of stats outputs
ballast_pam_Fo_stats <- data.frame(time_treatment = "t1_HW120",lab = "time: F1,89=19.29, p<0.001",time = "t1")
ballast_pam_Fo_stats2 <- data.frame(time_treatment = "t1_HW120",lab = "treat: F2,89=119.65, p<0.001",time = "t1")
ballast_pam_Fo_stats3 <- data.frame(time_treatment = "t1_HW120",lab = "int: F2,89=97.17, p<0.001",time = "t1")
...
ggsave("sctld ballast PAM Fo plot.pdf", plot= ballast_pam_Fo_plot, width=6, height=4, units="in", dpi=300)
...
ballast_pam_cld_Fv$Group  <- factor(ballast_pam_cld_Fv$Group , levels = c("t0_HW120","t0_DW24", "t0_DW120","t1_HW120","t1_DW24", "t1_DW120"))
...
# creates dummy dataframes for custom text labels of stats outputs
ballast_pam_Fv_stats <- data.frame(time_treatment = "t1_HW120",lab = "time: F1,89=174.7, p<0.001",time = "t1")
ballast_pam_Fv_stats2 <- data.frame(time_treatment = "t1_HW120",lab = "treat: F2,899=262.3, p<0.001",time = "t1")
ballast_pam_Fv_stats3 <- data.frame(time_treatment = "t1_HW120",lab = "int: F2,89=61.2, p<0.001",time = "t1")
...
ballast_pam_Fv_plot <-
  ...
  # scale_fill_discrete(name = "Treatment", labels = c("t0_HW", "t0_DW", "t0_DW","t1_HW", "t1_DW", "t1_DW")) +
  labs(x = "Time",
           y = "Fv",
           fill = 'Treatment') + 
  facet_grid(. ~ time, scales="free", space="free", switch = c("both")) +
...
ggsave("sctld ballast PAM Fv plot.pdf", plot= ballast_pam_Fv_plot, width=6, height=4, units="in", dpi=300)

# cell counts/PAM multiplot
multiplot=grid.arrange(ballast_count_abundance, ballast_counts_plot, ballast_pam_Fo_plot, ballast_pam_Fv_plot, ncol=1, nrow=4, widths=c(10), heights=c(4,4,4,4))

ggsave("sctld ballast watertesting panel.pdf", plot= multiplot, width=6, height=12, units="in", dpi=300)
